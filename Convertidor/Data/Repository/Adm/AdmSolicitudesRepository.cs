...
namespace Convertidor.Data.Repository.Adm
{
    public class AdmSolicitudesRepository:IAdmSolicitudesRepository
    {
        ...
        public async Task<string> UpdateMontoEnLetras(int codigoSolicitud)
        {
            ...
            try
            {
                FormattableString xqueryDiario = $"UPDATE ADM_SOLICITUDES  SET MONTO_LETRAS= UPPER(SIS.SIS_MONTOESCRITO((SELECT sum(TOTAL+TOTAL_MAS_IMPUESTO)  FROM adm.ADM_DETALLE_SOLICITUD WHERE ADM_DETALLE_SOLICITUD.CODIGO_SOLICITUD =ADM_SOLICITUDES.CODIGO_SOLICITUD),2)) WHERE ADM_SOLICITUDES.CODIGO_SOLICITUD ={codigoSolicitud}";
