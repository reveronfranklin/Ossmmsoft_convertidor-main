...
{
          public async Task<ResultDto<AdmOrdenPagoResponseDto>> Retornar(AdmOrdenPagoRetornar dto)
        {
            ...
            try
            {
                ...
                var validaRetornarOrdenPago =await ValidaRetornarOrdenPago(codigoOrdenPago,totalPagado);
                
                if (validaRetornarOrdenPago!="")
                {
                    result.Data = null;
                    result.IsValid = false;
                    result.Message =validaRetornarOrdenPago;
                    return result;
                }

                codigoOrdenPago.STATUS = "PE";
